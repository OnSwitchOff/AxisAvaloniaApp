...
namespace AxisAvaloniaApp.Rules.Exchange
{
    public class ShopTypeIsChosen : AbstractStage
    {
        ...
        {
            if (app == EExchanges.ExportToNAP && string.IsNullOrEmpty(settingsServise.AppSettings[Enums.ESettingKeys.OnlineShopType]))
            {
                await loggerService.ShowDialog("msgOnlineShopTypeIsNotChosen", "strAttention", UserControls.MessageBoxes.EButtonIcons.Warning);
